...
This app is a somewhat contrived BPM scenario that polls the customer DB used by lloyds demo to start a BPM process if the customers total purchases hit a certain threshold.  The BPM process calls a web service named getCustomerPreferences that returns the communications restrictions the customer has requested.
